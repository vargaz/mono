 ...
 * If mono_thread_info_is_async_context () is set, this is async safe. The returned MonoJitInfo might not have
...
lock_free_mempool_chunk_new (LockFreeMempool *mp)
{
...
lock_free_mempool_alloc0 (LockFreeMempool *mp, guint size)
{
	...
	chunk = mp->current;
	if (!chunk) {
		chunk = lock_free_mempool_chunk_new (mp);
		mono_memory_barrier ();
		/* Publish */
		mp->current = chunk;
	}
