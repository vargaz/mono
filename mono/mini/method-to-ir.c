...
static MonoMethodSignature *helper_sig_resume_unwind_trampoline = NULL;
		   ...
		   guint inline_offset, gboolean is_virtual_call)
{
	...
	GSList *finally_info_list = NULL, *l;
	...
	if (cfg->method == method) {
		...
		if (header->num_clauses) {
			...
			cfg->finally_info = g_hash_table_new (NULL, NULL);
		}
...
#ifdef MONO_ARCH_HAVE_RESUME_UNWIND
			if (clause->flags == MONO_EXCEPTION_CLAUSE_FINALLY) {
				FinallyInfo *info = mono_mempool_alloc0 (cfg->mempool, sizeof (FinallyInfo));

				g_hash_table_insert (cfg->finally_info, GUINT_TO_POINTER (clause->handler_offset), info);
				finally_info_list = g_slist_prepend_mempool (cfg->mempool, finally_info_list, info);

				info->finally_ind = mono_compile_create_var (cfg, &mono_defaults.int16_class->byval_arg, OP_LOCAL);
			}
#endif
	...
	/* Initialize finally indicator variables */
	for (l = finally_info_list; l; l = l->next) {
		FinallyInfo *info = l->data;

		MONO_EMIT_NEW_ICONST (cfg, info->finally_ind->dreg, 0);
	}
	...
	while (ip < end) {
		...
		case CEE_LEAVE_S: {
			GList *handlers;
			MonoBasicBlock *leave_target_bb;
			gboolean need_br = TRUE;
			...
			GET_BBLOCK (cfg, leave_target_bb, target);
			...
			if ((handlers = mono_find_final_block (cfg, ip, target, MONO_EXCEPTION_CLAUSE_FINALLY))) {
				...
				for (tmp = handlers; tmp; tmp = tmp->next) {
					...
					{
						GSList *bb_list;
						MonoBasicBlock *cont_bb;
						FinallyInfo *info;

						need_br = FALSE;

						info = g_hash_table_lookup (cfg->finally_info, GUINT_TO_POINTER (clause->handler_offset));
						g_assert (info);
						 ...
						 */
						MONO_EMIT_NEW_ICONST (cfg, info->finally_ind->dreg, (g_slist_length (bb_list) + 1));
						...
						if (tmp->next == NULL) {
							/* From the last handler, branch directly to the target bb */
							/* Branch opts can't do this since this is inside a clause */
							cont_bb = leave_target_bb;
						} else {
							/* Create a new bblock which ENDFINALLY will branch to */
							NEW_BBLOCK (cfg, cont_bb);
							MONO_START_BB (cfg, cont_bb);
							bblock = cfg->cbb;
						}

						info->call_handler_return_bbs = g_slist_append_mempool (cfg->mempool, info->call_handler_return_bbs, cont_bb);
					}
#else
			...
			if (need_br) {
				MONO_INST_NEW (cfg, ins, OP_BR);
				MONO_ADD_INS (bblock, ins);
				link_bblock (cfg, bblock, leave_target_bb);
				ins->inst_target_bb = leave_target_bb;
			}
			start_new_bblock = 1;
		...
		case CEE_ENDFINALLY: {
			...
			region = mono_find_block_region (cfg, bblock->real_offset);
			// FIXME: Add macros for this
			if ((region & (0xf << 4)) == MONO_REGION_FAULT) {
				MONO_INST_NEW (cfg, ins, OP_ENDFINALLY);
				MONO_ADD_INS (bblock, ins);
			} else {
#ifdef MONO_ARCH_HAVE_RESUME_UNWIND
				GSList *bb_list, *l;
				int ind_vreg, target_index;
				FinallyInfo *info;
				int clause_index;

				/*
				 * Branch to either the resume unwind code, or to the bblocks in info->call_handler_return_bbs.
				 */

				// FIXME: LEAVEs processed later (is that possible ?)

				region = mono_find_block_region (cfg, bblock->real_offset);
				// FIXME: Add macros for this
				g_assert ((region & (0xf << 4)) == MONO_REGION_FINALLY);
				clause_index = (region >> 8) - 1;

				info = g_hash_table_lookup (cfg->finally_info, GUINT_TO_POINTER (cfg->header->clauses [clause_index].handler_offset));
				g_assert (info);

				/* Load the indicator variable */
				g_assert (info->finally_ind);
				ind_vreg = alloc_ireg (cfg);
				MONO_EMIT_NEW_UNALU (cfg, OP_MOVE, ind_vreg, info->finally_ind->dreg);
				/* Reset the variable */
				MONO_EMIT_NEW_ICONST (cfg, info->finally_ind->dreg, 0);
				...
				target_index = 0;
				bb_list = info->call_handler_return_bbs;
				g_assert (g_slist_length (bb_list) < 65000);
				for (l = bb_list; l; l = l->next) {
					/* ind_vreg = x -> branch to bb_list [x - 1] */
					MONO_EMIT_NEW_BIALU_IMM (cfg, OP_ICOMPARE_IMM, -1, ind_vreg, target_index + 1);
					MONO_EMIT_NEW_BRANCH_BLOCK (cfg, OP_IBEQ, l->data);
					bblock = cfg->cbb;
					target_index ++;
				}
				/* ind_vreg == 0 -> branch to resume_unwind */
				mono_emit_abs_call (cfg, MONO_PATCH_INFO_INTERNAL_METHOD, "resume_unwind_trampoline", helper_sig_resume_unwind_trampoline, NULL);
#else
				MONO_INST_NEW (cfg, ins, OP_ENDFINALLY);
				MONO_ADD_INS (bblock, ins);
#endif
...
mono_create_helper_signatures (void)
{
	...
	helper_sig_resume_unwind_trampoline = mono_create_icall_signature ("void");
}
