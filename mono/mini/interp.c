/*
 * interp.c: CIL Interpreter for Mini
 *
 * Based on interp/interp.c:
 *
...
#define INIT_FRAME(frame,parent_frame,obj_this,method_args,method_retval,mono_method,rmethod) \
	do {	\
		(frame)->parent = (parent_frame);	\
		(frame)->obj = (obj_this);	\
		(frame)->stack_args = (method_args);	\
		(frame)->retval = (method_retval);	\
		(frame)->runtime_method = (rmethod) ? rmethod : mono_interp_get_runtime_method (mono_method); \
		(frame)->ex = NULL;	\
		(frame)->ip = NULL;	\
		(frame)->invoke_trap = 0;	\
	} while (0)
...
{
	const guint8* code = mono_method_get_header (mh)->code;
	fprintf (stderr, "Execution aborted in method: %s::%s\n", mh->klass->name, mh->name);
	fprintf (stderr, "Line=%d IP=0x%04x, Aborted execution\n", line,
			 (int)(ip-(const unsigned short *)code));
	g_print ("0x%04x %02x\n",
			 (int)(ip-(const unsigned short *)code), *ip);
	if (sp > stack)
		printf ("\t[%d] 0x%08x %0.5f\n", (int)(sp-stack), sp[-1].data.i, sp[-1].data.f);
}
...
#endif
	vt_sp = (guint8 *)sp + rtm->stack_size;
#if DEBUG_INTERP
	vtalloc = vt_sp;
#endif
...
mono_interp_enter (RuntimeMethod *rmethod, mgreg_t *regs, mgreg_t *fpregs, InterpResultBuf *res_buf)
{
	...
	info = mono_arch_interp_call_prepare (mono_method_signature (m));

	g_assert (mono_method_signature (m)->param_count < 16);

	mono_arch_interp_call_get_args (info, regs, fpregs, params);

	/* Based on interp_mono_runtime_invoke */
	frame.ex = NULL;
