 ...
 *
 *   Based on interp/transform.c, which is:
 *
		...
		case CEE_PREFIX1: 
			...
			case CEE_SIZEOF: {
				...
				if (mono_metadata_token_table (token) == MONO_TABLE_TYPESPEC) {
					...
					size = mono_type_size (type, &ialign);
				} else {
...
mono_interp_transform_method (InterpMethod *runtime_method, InterpThreadContext *context)
{
	...
	for (i = 0; i < signature->param_count; ++i) {
		...
		if (signature->pinvoke) {
			...
			size = mono_type_stack_size (signature->params [i], &ialign);
			align = ialign;
		}
		offset += align - 1;
		offset &= ~(align - 1);
