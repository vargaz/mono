...
ves_icall_get_trace (MonoException *exc, gint32 skip, MonoBoolean need_file_info)
{
	...
	for (i = skip; i < len; i++) {
		...
		if (GPOINTER_TO_INT (generic_info) & 0x1) {
			/* Interpreter frame */
			method = ip;
			ji = NULL;
		} else {
 ...
 */
static gboolean
mono_handle_exception_internal_first_pass (MonoContext *ctx, MonoObject *obj, gint32 *out_filter_idx, StackFrameInfo *out_frame, MonoJitInfo **out_ji, MonoJitInfo **out_prev_ji, MonoObject *non_exception)
{
	MonoError error;
	MonoDomain *domain = mono_domain_get ();
	MonoJitInfo *ji = NULL;
	...
	while (1) {
		...
		gboolean skip_trace = FALSE;
		if (method->wrapper_type) {
			if (method->wrapper_type == MONO_WRAPPER_RUNTIME_INVOKE) {
				skip_trace = TRUE;
			} else {
				WrapperInfo *winfo = mono_marshal_get_wrapper_info (method);
				if (winfo && winfo->subtype == WRAPPER_SUBTYPE_GSHAREDVT_OUT_SIG)
					/* Used to enter the interpreter */
					skip_trace = TRUE;
			}
		}

		if (!skip_trace && mono_ex) {
			// avoid giant stack traces during a stack overflow
			if (frame_count < 1000) {
				if (in_interp) {
					trace_ips = g_list_prepend (trace_ips, frame.method);
					/* Mark this as an interpreter frame */
					trace_ips = g_list_prepend (trace_ips, GINT_TO_POINTER (1));
				} else {
					gpointer generic_info = get_generic_info_from_stack_frame (ji, ctx);
					g_assert ((GPOINTER_TO_INT (generic_info) & 0x1) == 0);
		...
		for (i = clause_index_start; i < (ji ? ji->num_clauses : 0); i++) {
...
mono_handle_exception_internal (MonoContext *ctx, MonoObject *obj, gboolean resume, MonoJitInfo **out_ji)
{
	...
	if (!resume) {
		...
		res = mono_handle_exception_internal_first_pass (&ctx_cp, obj, &first_filter_idx, &frame, &ji, &prev_ji, non_exception);
	...
	while (1) {
		...
		if (resume) {
			...
			if (frame.type == FRAME_TYPE_INTERP_TO_MANAGED) {
				 ...
				 * return value register etc. is not set, so has to be careful.
				 */
